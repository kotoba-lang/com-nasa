(ns nasa.main
  "Kotodama WASM entrypoint for the Nasa clean-room actor (L4) — Clojure port.
  ...
  facts (`nasa.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Filing"       :plural "filings"       :id-prefix "nasa_fil"
    :fields [:filerId :form :period :status]      :required [:form :period]
    :coerce {}                                     :refs {}}
   {:entity "Entity"       :plural "entities"      :id-prefix "nasa_ent"
    :fields [:name :ein :jurisdiction]            :required [:name :ein]
    :coerce {}                                     :refs {}}
   {:entity "Dataset"      :plural "datasets"      :id-prefix "nasa_dat"
    :fields [:name :agency :updatedAt]            :required [:name :agency]
    :coerce {}                                     :refs {}}
   {:entity "Application"  :plural "applications"  :id-prefix "nasa_app"
    :fields [:applicantId :program :status]       :required [:program :status]
    :coerce {}                                     :refs {}}
   {:entity "PublicRecord" :plural "publicrecords" :id-prefix "nasa_pub"
    :fields [:recordType :jurisdiction :recordedAt] :required [:recordType :jurisdiction]
    :coerce {}                                     :refs {}}
   {:entity "Notice"       :plural "notices"       :id-prefix "nasa_not"
...
(defn emit-facts
  "EAVT facts for one record: {\"nasa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nasa-compat" :tier tier :entities entities} 200])
